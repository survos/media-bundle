...
#[AsCommand(
    name: 'media:fetch',
    description: 'Fetch media metadata from external providers'
)]
final class FetchMediaCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->info("Fetching media from {$providerName}...");
        ...
        try {
            if ($input->getOption('dry-run')) {
                ...
                $io->success("Fetched " . count($synced) . " media items from {$providerName}");
            }
        } catch (\Exception $e) {
