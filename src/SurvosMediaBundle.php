...
class SurvosMediaBundle extends AbstractSurvosBundle
{
    ...
    {
        // NOTE: HasConfigurableRoutes was mixed in solely to expose MediaCallbackController's
        // /media/callback route, and went away with it. mediary's callback now arrives at
        // /webhook/mediary, which is FrameworkBundle's own WebhookController — the app imports
        // that route itself (see docs/webhooks.md) and this bundle contributes only a request
        // parser and a consumer. src/Controller/ no longer exists.
        $definition->rootNode()
            ->children()
